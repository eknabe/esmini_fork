...
3. For all remaining object types, esmini tries to map the subtype to an OSI stationary-object type. The subtype comparison is case-insensitive; `TYPE_` is prepended when it is not already present. For example, subtype `building` maps to `TYPE_BUILDING`.
...
This schema overcomes limitations from OpenDRIVE and OSI type incompatibility. For example it allows for emitting TYPE_DELINEATOR which has no corresponding road object type. Example:
