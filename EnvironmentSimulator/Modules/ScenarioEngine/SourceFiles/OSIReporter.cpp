...
int OSIReporter::UpdateOSIMovingObject(ObjectState *objectState)
{
    ...
    if (objectState->state_.info.obj_type == static_cast<int>(Object::Type::VEHICLE))
    {
        unsigned int n_wheels = objectState->state_.info.wheel_data.size();
        ...
        for (int i = 0; i < static_cast<int>(n_wheels); i++)
        {
            ...
            obj_osi_internal.mobj->mutable_vehicle_attributes()->mutable_wheel_data(i)->set_axle(objectState->state_.info.wheel_data[i].axle);
            obj_osi_internal.mobj->mutable_vehicle_attributes()->mutable_wheel_data(i)->set_index(
                objectState->state_.info.wheel_data[i].index);  // Index along axis
        }
    }
